...
class EnemyRobot:
    def __init__(self):
        ...
        self.healthfinder_sub = rospy.Subscriber("/enemy/healthfinder", BoundingBox3d, self.healthfinder_callback)
        self.playerfinder_sub = rospy.Subscriber("/enemy/playerfinder", BoundingBox3d, self.playerfinder_callback)
        self.cannon_pub = rospy.Publisher("/enemy/cannon", String, queue_size=10)
        self.scan_sub = rospy.Subscriber("/enemy/scan", LaserScan, self.scan_callback)
        self.odom_sub = rospy.Subscriber("/enemy/odom", PoseWithCovarianceStamped, self.odom_callback)
        self.canshoot_sub = rospy.Subscriber("/enemy/canshoot", Int32, self.canshoot_callback)
        self.hp_sub = rospy.Subscriber("/enemy/hp", Int32, self.hp_callback)

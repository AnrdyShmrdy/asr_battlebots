...
class PlayerRobot:
    def __init__(self):
        ...
        self.playerfinder_sub = rospy.Subscriber("/ponce/playerfinder", BoundingBox3d, self.playerfinder_callback)
        self.cannon_pub = rospy.Publisher("/ponce/cannon", String, queue_size=10)
        self.scan_sub = rospy.Subscriber("/ponce/scan", LaserScan, self.scan_callback)
        self.odom_sub = rospy.Subscriber("/ponce/odom", PoseWithCovarianceStamped, self.odom_callback)
        self.canshoot_sub = rospy.Subscriber("/ponce/canshoot", Int32, self.canshoot_callback)
        self.hp_sub = rospy.Subscriber("/ponce/hp", Int32, self.hp_callback)
